...
class MultiTopicBroker(publishersAddress: InetSocketAddress, subscribersAddress: InetSocketAddress,
  ...
  override val materializer = ActorMaterializer(
    ActorMaterializerSettings(system)
      .withInputBuffer(initialSize = 1024, maxSize = 2048)
  ...
  override def run(): Future[Unit] = {
    ...
    val inF = in.runForeach({ con ⇒
      system.log.info("New topics writer from: {}", con.remoteAddress)

      val framer = new Framers()
      val inNetworkSubscriber = ActorSubscriber[Tweet](system.actorOf(TopicsWriter.props(topics, con.remoteAddress)))
    ...
    val outF = out.runForeach({ con ⇒
      system.log.info("New topic reader from: {}", con.remoteAddress)

      val sourceN = Source.fromGraph(
    ...
    handleIOFailure(inF, "In network error", Some(promise))
    handleIOFailure(outF, "Out network error", Some(promise))
    promise.future
  }
}